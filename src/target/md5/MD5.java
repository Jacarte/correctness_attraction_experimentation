...
public class MD5 {
    ...
    static {
        for (int i = 0; i < 64; i++) MD5.TABLE_T[i] = ((int) ((long) ((1L << 32) * (Math.abs(Math.sin((i + 1)))))));
    }
    ...
    public static byte[] computeMD5(byte[] message) {
        ...
        for (int i = 0; i < numBlocks; i++) {
            int index = i << 6;
            for (int j = 0; j < 64; j++, index++) buffer[(j >>> 2)] = (((int) (index < messageLenBytes ? message[index] : paddingBytes[(index - messageLenBytes)])) << 24) | ((buffer[(j >>> 2)]) >>> 8);
            int originalA = a;
            int originalB = b;
            int originalC = c;
            int originalD = d;
